...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_temporal_features', action='store_true', default=True,
                        help='Enable absolute/relative temporal feature embeddings '
                             '(seq hour, weekday, diff-hours/days/weeks + target hour/weekday NS token). '
                             'Adds 1 extra NS token. Default on.')
    parser.add_argument('--no_temporal_features', dest='use_temporal_features',
                        action='store_false',
                        help='Disable the temporal feature embeddings')
    ...
    parser.add_argument('--csa_top_k', type=int, default=0,
                        help='CSA (Content Sparse Attention) top-k: in each Cross-Attention '
                             'step, only retain the top-k seq tokens ranked by dot-product '
