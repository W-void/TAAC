...
class ModelInput(NamedTuple):
    ...
    # ---- new temporal fields (all optional; None means feature disabled) ----
    req_hour: Optional[torch.Tensor] = None           # (B,) 1-24, 0=pad
    req_weekday: Optional[torch.Tensor] = None        # (B,) 1-7,  0=pad
    req_week_of_year: Optional[torch.Tensor] = None   # (B,) 1-53, 0=pad
    seq_hours: Optional[dict] = None               # {domain: (B, L)}
    seq_weekdays: Optional[dict] = None            # {domain: (B, L)}
    seq_diff_hours: Optional[dict] = None          # {domain: (B, L)}
    seq_diff_days: Optional[dict] = None           # {domain: (B, L)}
    seq_diff_weeks: Optional[dict] = None          # {domain: (B, L)}
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        # Per-sequence LayerNorm applied after MeanPool to stabilize pooled vector
        # scale (especially important for short sequences where variance is high)
        self.seq_pool_norms = nn.ModuleList([
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            ...
            # LayerNorm to stabilize pooled vector scale (critical for short seqs)
            seq_pooled = self.seq_pool_norms[i](seq_pooled)  # (B, D)
...
class DynamicQueryRefiner(nn.Module):
    """Layer-wise Q refresh (r1): regenerates per-sequence Q tokens from the
    ...
    Args:
        ...
        num_queries: Number of Q tokens per sequence.
        num_sequences: Number of sequences.
        num_ns: Number of NS tokens (for ns_mean pooling input dim).
    """
    ...
    def __init__(
        ...
        # Domain bias embedding: one learnable vector per sequence domain
        # (r4) gives each domain's Q a semantic head-start
        self.domain_embs = nn.Embedding(num_sequences, d_model)

        # Per-sequence LayerNorm on the pooled context before Q generation
    ...
    def forward(
        ...
        """Generates refreshed Q tokens from current NS tokens.

        Args:
            ns_tokens: (B, Nns, D) — current (already updated) NS tokens.
            seq_tokens_list: List of (B, L_i, D).
            seq_padding_masks: List of (B, L_i), True = padding.

        Returns:
            List of (B, Nq, D) refreshed Q token tensors.
        """
        B = ns_tokens.shape[0]
        device = ns_tokens.device

        # NS mean pool: (B, D)
        ns_mean = ns_tokens.mean(dim=1)  # (B, D)

        q_list = []
        for i in range(self.num_sequences):
            # Sequence mean pool (masked)
            valid = (~seq_padding_masks[i]).unsqueeze(-1).float()  # (B, L, 1)
            seq_mean = (seq_tokens_list[i] * valid).sum(dim=1) / valid.sum(dim=1).clamp(min=1)

            # Domain bias (r4)
            d_idx = torch.full((B,), i, dtype=torch.long, device=device)
            domain_bias = self.domain_embs(d_idx)  # (B, D)

            # Context = NS mean + seq mean + domain bias
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    Enhancements:
      r1/r4: DynamicQueryRefiner regenerates Q from current NS tokens each
             block (layer-wise Q refresh + domain bias embedding).
      r2:    CSA top-k mask: cross-attention only attends the top-k most
             relevant seq tokens (by dot-product similarity with Q mean).
      r3:    protect_seq: when True the seq encoder does NOT receive NS tokens
             as query context; used in the last block to prevent SCP.
    """
    ...
    def __init__(
        ...
        csa_top_k: int = 0,        # r2: 0 = disabled; >0 = keep only top-k seq tokens per Q
        protect_seq: bool = False,  # r3: SCP guard for last block
    ) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.num_queries = num_queries
        ...
        # r1/r4: DynamicQueryRefiner — one per block, refreshes Q each layer
        self.dyn_q_refiner = DynamicQueryRefiner(
            d_model=d_model,
    ...
    def _csa_mask(
        ...
        """r2: Builds a boolean attention mask keeping only top-k seq tokens.

        Uses mean-Q dot-product similarity to rank seq tokens by relevance.

        Args:
            q_tokens: (B, Nq, D)
            seq_tokens: (B, L, D)
            seq_padding_mask: (B, L), True = padding
            top_k: number of seq tokens to retain per sample

        Returns:
            mask: (B, L), True = masked-out (padding OR low relevance)
        """
        B, L, D = seq_tokens.shape
        # Mean Q as query signal: (B, D)
        q_mean = q_tokens.mean(dim=1)  # (B, D)
        # Cosine-style similarity (dot product; embeddings are LN'd so scale is ~1)
        sim = torch.bmm(seq_tokens, q_mean.unsqueeze(-1)).squeeze(-1)  # (B, L)
        # Mask padding positions with -inf so they never win top-k
        sim = sim.masked_fill(seq_padding_mask, float('-inf'))
        # Find threshold: top_k-th score per sample
        actual_k = min(top_k, L)
        topk_vals, _ = sim.topk(actual_k, dim=1)             # (B, actual_k)
        threshold = topk_vals[:, -1].unsqueeze(1)             # (B, 1)
        # Tokens below threshold (and padding) are masked out
        csa_mask = (sim < threshold) | seq_padding_mask       # (B, L)
        return csa_mask
    ...
    def forward(
        ...
        # r1/r4: Layer-wise Q refresh — use current NS tokens to regenerate Q
        # before Query Decoding (instead of reusing stale Q from previous block)
        if not self.protect_seq:
            # Normal blocks: refresh Q from updated NS tokens
            q_tokens_list = self.dyn_q_refiner(
                ns_tokens, seq_tokens_list, seq_padding_masks
            )
        # protect_seq (last block, r3): keep Q as-is to avoid NS→seq contamination
        ...
        # 2. Independent Query Decoding per sequence (with optional CSA mask)
        decoded_qs = []
        for i in range(S):
            rc = rope_cos_list[i] if rope_cos_list is not None else None
            rs = rope_sin_list[i] if rope_sin_list is not None else None
            ...
            if self.csa_top_k > 0 and not self.protect_seq:
                ...
                csa_mask = next_masks[i]

            decoded_q_i = self.cross_attns[i](
                q_tokens_list[i], next_seqs[i], csa_mask,
                rope_cos=rc, rope_sin=rs,
            )
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 id_threshold: int = 0,
                 id_dropout_rate: float = 0.0) -> None:
        super().__init__()
        ...
        # High-cardinality ID dropout (same mechanism as seq_id_emb_dropout)
        self._is_id = [
            id_threshold > 0 and int(vs) > id_threshold
            for vs, _, _ in feature_specs
        ]
        self.id_emb_dropout = nn.Dropout(id_dropout_rate) if id_dropout_rate > 0 else None
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                if fid_mask is not None and not fid_mask[fid_idx]:
                    ...
                    # High-cardinality ID dropout during training
                    if (self.id_emb_dropout is not None
                            and self._is_id[fid_idx]
                            and self.training):
                        fid_emb = self.id_emb_dropout(fid_emb)
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            id_threshold: Features with vocab > this are treated as ID features
                and receive id_dropout during training (0 = disabled).
            id_dropout_rate: Dropout probability applied to high-cardinality
                ID embeddings during training.
        """
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # NS-side ID dropout rate: twice the base dropout (same as seq_id_emb_dropout)
        ns_id_dropout_rate = dropout_rate * 2
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                id_dropout_rate=ns_id_dropout_rate,
            )
        ...
        self.dense_input_dropout = nn.Dropout(p=dropout_rate)
        ...
        # +1 for the target temporal token (hour + weekday fused into one token)
        self.num_ns = (num_user_ns + (1 if self.has_user_dense else 0)
                       + num_item_ns + (1 if self.has_item_dense else 0)
                       + (1 if use_temporal_features else 0))
        ...
        # ================== Temporal Feature Embeddings (optional) ==================
        if use_temporal_features:
            from dataset import (
                NUM_HOUR_BUCKETS, NUM_WEEKDAY_BUCKETS, NUM_WEEK_OF_YEAR_BUCKETS,
                NUM_HOUR_DIFF_BUCKETS, NUM_DAY_DIFF_BUCKETS, NUM_WEEK_DIFF_BUCKETS,
            )
            # Target (request) time embeddings: hour + weekday + week_of_year
            # Each has its own embedding table; concat(3 * d_model) -> Linear -> 1 NS token
            self.req_hour_emb = nn.Embedding(NUM_HOUR_BUCKETS, d_model, padding_idx=0)
            self.req_weekday_emb = nn.Embedding(NUM_WEEKDAY_BUCKETS, d_model, padding_idx=0)
            self.req_week_of_year_emb = nn.Embedding(NUM_WEEK_OF_YEAR_BUCKETS, d_model, padding_idx=0)
            # Concat 3 embeddings then project: 3*d_model -> d_model
            self.req_time_proj = nn.Sequential(
                nn.Linear(3 * d_model, d_model),
                nn.LayerNorm(d_model),
            )
            # Sequence absolute-time embeddings (added into seq token, same d_model)
            self.seq_hour_emb = nn.Embedding(NUM_HOUR_BUCKETS, d_model, padding_idx=0)
            self.seq_weekday_emb = nn.Embedding(NUM_WEEKDAY_BUCKETS, d_model, padding_idx=0)
            # Sequence relative-diff embeddings
            self.seq_diff_hour_emb = nn.Embedding(NUM_HOUR_DIFF_BUCKETS, d_model, padding_idx=0)
            self.seq_diff_day_emb = nn.Embedding(NUM_DAY_DIFF_BUCKETS, d_model, padding_idx=0)
        ...
        # semantically heterogeneous tokens are not forced to share parameters.
        # Applied once on the initial NS tokens before feeding into HyFormer blocks.
        def _make_ns_ffn(in_d: int) -> nn.Sequential:
            return nn.Sequential(
                nn.LayerNorm(in_d),
                nn.Linear(in_d, in_d * hidden_mult),
                nn.SiLU(),
                nn.Linear(in_d * hidden_mult, in_d),
            )
        self.user_ns_ffn = _make_ns_ffn(d_model)
        self.item_ns_ffn = _make_ns_ffn(d_model)
        if self.has_user_dense:
            self.user_dense_ffn = _make_ns_ffn(d_model)
        if self.has_item_dense:
            self.item_dense_ffn = _make_ns_ffn(d_model)
        if use_temporal_features:
            self.temporal_ns_ffn = _make_ns_ffn(d_model)
        ...
        # r3: last block uses protect_seq=True to guard against SCP collapse
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                csa_top_k=csa_top_k,
                protect_seq=(blk_idx == num_hyformer_blocks - 1),  # r3
            )
            for blk_idx in range(num_hyformer_blocks)
        ])
        self.csa_top_k = csa_top_k
        ...
        # Output projection
        # Input dim = Q tokens (Nq * S * D) + NS tokens (Nns * D)
        # Both carry complementary information after multi-layer iteration.
        output_in_dim = (num_queries * self.num_sequences + self.num_ns) * d_model
        self.output_proj = nn.Sequential(
            nn.Linear(output_in_dim, d_model),
            nn.LayerNorm(d_model),
        )

        # r5: Global skip connection — projects initial NS tokens directly to D
        # and adds to final output (bypasses all blocks, preserves low-level signal)
        self.global_skip_proj = nn.Sequential(
    ...
    def _embed_seq_domain(
        ...
        seq_weekday_ids: Optional[torch.Tensor] = None,
    ...
    def _run_multi_seq_blocks(
        ...
        # r5: cache initial NS token flat vector before any block updates it
        B = ns_tokens.shape[0]
        ...
        #         NS tokens (carry static-feature info enriched by blocks)
        all_q = torch.cat(curr_qs, dim=1)                    # (B, Nq*S, D)
        all_tokens = torch.cat([all_q, curr_ns], dim=1)      # (B, Nq*S + Nns, D)
        output = all_tokens.view(B, -1)                       # (B, (Nq*S + Nns)*D)
