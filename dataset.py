...
# Hour-of-day bucket: 0=padding, 1..24 (hour 0 -> bucket 1, ..., hour 23 -> bucket 24)
NUM_HOUR_BUCKETS = 25       # 0(pad) + 24 hours

# Day-of-week bucket: 0=padding, 1..7 (Mon=0 -> 1, ..., Sun=6 -> 7)
NUM_WEEKDAY_BUCKETS = 8     # 0(pad) + 7 days

# Quarter-of-year bucket: 0=padding, 1..4  (week_of_year % 4 + 1 -> 粗粒度季节信号，泛化性更好)
NUM_WEEK_OF_YEAR_BUCKETS = 5    # 0(pad) + 4 quarters

# ─────────────────────── Relative-time diff buckets ─────────────────────────

# Hour-diff boundaries (unit: hours).  0=padding, 1..N.
# Covers 0-1h, 1-2h, ..., 12h, 24h, 48h, 72h, 7d, 14d, 30d, >30d
HOUR_DIFF_BOUNDARIES = np.array([
    1, 2, 3, 4, 6, 8, 12, 18, 24, 36, 48, 72,
    96, 120, 144, 168,           # 4d..7d
    336, 504, 672,               # 14d, 21d, 28d
    720, 1440,                   # 30d, 60d
], dtype=np.int64)
NUM_HOUR_DIFF_BUCKETS = len(HOUR_DIFF_BOUNDARIES) + 1   # +1 for padding slot 0

# Day-diff boundaries (unit: days).  0=padding, 1..N.
DAY_DIFF_BOUNDARIES = np.array([
    1, 2, 3, 4, 5, 6, 7,
    10, 14, 21, 28,
    45, 60, 90, 180, 365,
], dtype=np.int64)
NUM_DAY_DIFF_BUCKETS = len(DAY_DIFF_BOUNDARIES) + 1

# Week-diff boundaries (unit: weeks).  0=padding, 1..N.
WEEK_DIFF_BOUNDARIES = np.array([
    1, 2, 3, 4, 6, 8, 12, 26, 52,
], dtype=np.int64)
NUM_WEEK_DIFF_BUCKETS = len(WEEK_DIFF_BOUNDARIES) + 1


def _ts_to_hour(ts: np.ndarray) -> np.ndarray:
    """Unix timestamp (seconds) -> hour-of-day bucket id (1-24; 0=padding when ts==0)."""
    result = np.zeros_like(ts, dtype=np.int64)
    valid = ts > 0
    result[valid] = (ts[valid] % 86400) // 3600 + 1   # 0..23 -> 1..24
    return result


def _ts_to_weekday(ts: np.ndarray) -> np.ndarray:
    """Unix timestamp (seconds) -> weekday bucket id (1-7; 0=padding when ts==0).
    1970-01-01 is Thursday (weekday=3 in Python datetime), so:
        days_since_epoch % 7 gives 0=Thu, 1=Fri, ..., 6=Wed
    We remap to Mon=1 .. Sun=7.
    """
    result = np.zeros_like(ts, dtype=np.int64)
    valid = ts > 0
    # days_since_epoch % 7: 0=Thu,1=Fri,2=Sat,3=Sun,4=Mon,5=Tue,6=Wed
    raw = (ts[valid] // 86400) % 7
    # shift so Mon=0: (raw - 4) % 7  =>  Mon=0..Sun=6, then +1 => Mon=1..Sun=7
    result[valid] = (raw - 4) % 7 + 1
    return result


def _ts_to_week_of_year(ts: np.ndarray) -> np.ndarray:
    """Unix timestamp (seconds) -> quarter-of-year bucket id (1-4; 0=padding when ts==0).

    Originally returned ISO week (1-53); now returns quarter to improve
    generalisation across train/test time splits.  The function name is kept
    unchanged so callers and ModelInput fields do not need to be renamed.

    Mapping: month 1-3 -> Q1=1, 4-6 -> Q2=2, 7-9 -> Q3=3, 10-12 -> Q4=4.
    Approximation: day_of_year (mod 365) // 91 + 1, capped at 4.
    """
    result = np.zeros_like(ts, dtype=np.int64)
    valid = ts > 0
    days_since_epoch = ts[valid] // 86400
    # approximate day-of-year (ignores leap years)
    day_of_year = days_since_epoch % 365
    quarter = np.clip(day_of_year // 91 + 1, 1, 4)
    result[valid] = quarter
    return result
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_seq_diff_hours = {}
        self._buf_seq_diff_days = {}
        self._buf_seq_diff_weeks = {}
        for domain in self.seq_domains:
            ...
            self._buf_seq_diff_hours[domain] = np.zeros((B, max_len), dtype=np.int64)
            self._buf_seq_diff_days[domain] = np.zeros((B, max_len), dtype=np.int64)
            self._buf_seq_diff_weeks[domain] = np.zeros((B, max_len), dtype=np.int64)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # Absolute time features for target (request) timestamp
        req_hour = _ts_to_hour(timestamps)               # (B,) values 1-24, 0=invalid
        req_weekday = _ts_to_weekday(timestamps)         # (B,) values 1-7,  0=invalid
        req_week_of_year = _ts_to_week_of_year(timestamps)  # (B,) values 1-53, 0=invalid
        if self.is_training:
        ...
        result = {
            ...
            # Target (request) absolute time features
            'req_hour': torch.from_numpy(req_hour),                   # (B,) 1-24, 0=pad
            'req_weekday': torch.from_numpy(req_weekday),             # (B,) 1-7,  0=pad
            'req_week_of_year': torch.from_numpy(req_week_of_year),   # (B,) 1-53, 0=pad
        }
        ...
        for domain in self.seq_domains:
            ...
            # ---- Absolute time features for sequence (hour, weekday) ----
            seq_hour = self._buf_seq_hour[domain][:B]
            seq_weekday = self._buf_seq_weekday[domain][:B]
            seq_hour[:] = 0
            seq_weekday[:] = 0

            seq_diff_hours = self._buf_seq_diff_hours[domain][:B]
            seq_diff_days = self._buf_seq_diff_days[domain][:B]
            seq_diff_weeks = self._buf_seq_diff_weeks[domain][:B]
            seq_diff_hours[:] = 0
            seq_diff_days[:] = 0
            seq_diff_weeks[:] = 0

            if ts_ci is not None:
                # ts_padded already computed above (shape B, max_len)
                # Absolute time: hour and weekday of each historical action
                valid_mask = ts_padded > 0   # (B, max_len)
                flat_ts = ts_padded.ravel()  # (B*max_len,)

                flat_hour = _ts_to_hour(flat_ts).reshape(B, max_len)
                flat_weekday = _ts_to_weekday(flat_ts).reshape(B, max_len)
                # Zero out padding positions
                flat_hour[~valid_mask] = 0
                flat_weekday[~valid_mask] = 0
                seq_hour[:] = flat_hour
                seq_weekday[:] = flat_weekday

                # Relative time diff in hours / days / weeks
                # time_diff already computed above (B, max_len), unit=seconds, >=0
                diff_h = np.maximum(time_diff // 3600, 0)   # seconds -> hours
                diff_d = np.maximum(time_diff // 86400, 0)  # seconds -> days
                diff_w = np.maximum(time_diff // 604800, 0) # seconds -> weeks

                def _bucketize(arr, boundaries):
                    """Map values to bucket ids 1..N; 0 reserved for padding."""
                    raw = np.clip(
                        np.searchsorted(boundaries, arr.ravel()),
                        0, len(boundaries) - 1,
                    ).reshape(arr.shape) + 1
                    raw[ts_padded == 0] = 0  # restore padding
                    return raw

                seq_diff_hours[:] = _bucketize(diff_h, HOUR_DIFF_BOUNDARIES)
                seq_diff_days[:] = _bucketize(diff_d, DAY_DIFF_BOUNDARIES)
                seq_diff_weeks[:] = _bucketize(diff_w, WEEK_DIFF_BOUNDARIES)

            result[f'{domain}_seq_hour'] = torch.from_numpy(seq_hour.copy())
            result[f'{domain}_seq_weekday'] = torch.from_numpy(seq_weekday.copy())
            result[f'{domain}_diff_hours'] = torch.from_numpy(seq_diff_hours.copy())
            result[f'{domain}_diff_days'] = torch.from_numpy(seq_diff_days.copy())
            result[f'{domain}_diff_weeks'] = torch.from_numpy(seq_diff_weeks.copy())
